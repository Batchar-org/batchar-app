...
import { createAdminClient, generateTempPassword } from '../_shared/supabase.ts';
...
Deno.serve(async (req) => {
  ...
  if (!user) {
    return errorResponse(req, 'USER_NOT_FOUND', '사용자를 찾을 수 없습니다.', 404);
  }
  ...
  try {
    await sendEmail(email, '[BatChar] 임시 비밀번호 발급', buildTempPasswordHtml(tempPassword));
  } catch (e) {
